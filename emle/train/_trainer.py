...
class EMLETrainer:
    ...
    def train(
        ...
        c6: array or tensor or list of tensor/arrays of shape (N_BATCH, N_ATOMS)
            Atomic exchange-hole multipole moments.
        ...
        l2_reg_c6: float
            L2 regularization coefficient for C6 model training.
            Only relevant if atomic exchange-hole multipole moment targets are provided.
        ...
        if not (train_qeq or train_thole or train_c6):
            raise ValueError(
                "At least one of (q_core and q_val), alpha, or c6 must be provided."
            )

        if (train_thole or train_c6) and not train_qeq and q_mol is None:
            raise ValueError(
                "q_mol must be provided when training Thole or C6 without QEq data."
            )
        ...
        if train_c6:
            c6_train = _pad_to_max(c6)[train_mask]
        ...
        if not emle_model:
            ...
            params = {
                ...
                * _torch.ones(len(species), dtype=dtype, device=_torch.device(device)),
                ...
                "c6_Z": _torch.ones(
                    len(species), dtype=dtype, device=_torch.device(device)
                ),
                "ref_values_c6": (
                    _torch.ones(
                        *ref_values_s.shape,
            ...
            emle_base = self._emle_base(
                ...
                dispersion_mode="c6" if train_c6 else None,
                device=_torch.device(device),
                dtype=dtype,
            )
        else:
        ...
        if train_c6:
            _logger.info("Fitting c6 values...")
            param_names = [name for name, _ in emle_base.named_parameters()]
            if "c6_Z" not in param_names or "ref_values_c6" not in param_names:
                # Initialize c6 parameters if they do not exist yet.
                emle_base.c6_Z = _torch.nn.Parameter(
                    _torch.ones(
                        len(emle_base._species),
                        dtype=dtype,
                        device=_torch.device(device),
                    )
                )
            ...
            _logger.debug(f"Optimized c6_Z: {emle_base.c6_Z.data}")
        ...
        if alpha_mode == "reference":
            ...
            _, _, _, A_thole, *_ = emle_base(
                z.to(device=device, dtype=_torch.int64),
                xyz.to(device=device, dtype=dtype),
                q_mol,
            )
            plot_data["alpha_reference"], _ = self._thole_loss._get_alpha_mol(
                A_thole, z_mask
            )
